  ...
  actions: {
    setTheme(theme: 'light' | 'dark' | 'eyecare-light') {
      ...
      if (typeof window !== 'undefined') {
        ...
        document.documentElement.classList.remove('dark', 'eyecare', 'eyecare-dark')
        document.body.classList.remove('eye-care-mode', 'eye-care-dark-mode')
    ...
    toggleQuickSettings() {
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem('quickSettingsVisible', this.quickSettingsVisible.toString())
      }
    },
    ...
    initSettings() {
      if (typeof window !== 'undefined') {
        ...
        // Load line height
        const savedLineHeight = parseFloat(localStorage.getItem('lineHeight') || '1.6')
